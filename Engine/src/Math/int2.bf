namespace Engine
{
	...
	public struct int2 : IHashable
	{
		...
		public this(Vector2 uv)
		{
			values = .(int(uv.x), int(uv.y));
		}
		...
		/// </summary>
		public static implicit operator Vector2(int2 v) => Vector2((float)v.x, (float)v.y);
		...
		/// </summary>
		public static explicit operator Vector3(int2 v) => Vector3((float)v.x, (float)v.y, 0f);

		/// <summary>
		/// Explicitly converts this to a float4. (Higher components are zeroed)
		/// </summary>
		public static explicit operator Vector4(int2 v) => Vector4((float)v.x, (float)v.y, 0f, 0f);
